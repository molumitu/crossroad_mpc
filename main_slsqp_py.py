...
def run_mpc():
    ...
    for name_index in range(step_length):
        ...
        results = minimize(
                            lambda u: mpc_cost_function(u, ego_list, future_ref_array, horizon, STEP_TIME, Q, R), # python_function
                            # lambda u: mpc_cpp.mpc_cost_function(u, ego_list, vehicles_xy_array_static, future_ref_array, Q, R, P),
                            #lambda u: mpc_wrapper(u),
                            x0 = tem_action_array.flatten(),
                            method = 'SLSQP',
                            bounds = bounds,
        ...
        tem_action_array = np.concatenate((results.x[2:],results.x[-2:]),axis =0)
    ...
    np.savetxt(f'compare_solver_result/slsqp_py_result{current_time:%Y_%m_%d_%H_%M_%S}.csv', record_result, delimiter = ',')
    np.savetxt(f'compare_solver_result/slsqp_py_time{current_time:%Y_%m_%d_%H_%M_%S}.csv', time_list)
